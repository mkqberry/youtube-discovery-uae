...
class ProductionSegmenter:
    ...
    def _collect_audio_groups(self, audio_path: str) -> Dict[str, List[Path]]:
        """Verilen yoldan ses dosyalarını toplar ve alt klasörlere göre gruplar.

        Eğer yol bir dosyaysa, '.' anahtarıyla tek elemanlı grup döner.
        Eğer yol bir klasörse, alt klasörler dahil tüm .wav dosyalarını
        bulur ve her alt klasör için ayrı grup oluşturur.

        Args:
            audio_path: Ses dosyası veya WAV dosyaları içeren klasör yolu.

        Returns:
            Alt klasör relative yolu -> WAV dosya listesi sözlüğü.
            Örnek: {"3FNwJeiMRpE": [Path(...), ...], "e440H6C4D-I": [Path(...)]}
        """
        path = Path(audio_path)
        groups: Dict[str, List[Path]] = {}

        if path.is_file():
            # Tek dosya: '.' grubu altında döner
            groups["."] = [path]
            return groups
        ...
        if path.is_dir():
            ...
            if not wav_files:
                logger.warning(f"Klasörde WAV dosyası bulunamadı: {path}")
                return groups

            logger.info(f"Klasörde {len(wav_files)} WAV dosyası bulundu: {path}")

            # Her dosyayı input kökünden relative yoluna göre grupla
            for wav_file in wav_files:
                rel_parent = str(wav_file.relative_to(path).parent)
                # "." kök dizin demek (alt klasör yok)
                groups.setdefault(rel_parent, []).append(wav_file)

            logger.info(f"{len(groups)} alt klasör grubu oluşturuldu: {list(groups.keys())}")
            return groups

        logger.error(f"Geçersiz yol (dosya veya klasör değil): {audio_path}")
        return groups
    ...
    def _process_single_file(
        ...
        """Tek bir ses dosyasını işler ve segmentleri çıktı klasörüne yazar.

        Args:
            audio_file: İşlenecek ses dosyası.
            clips_dir: Segment wav dosyalarının yazılacağı klasör.
            f_out: metadata.jsonl dosya nesnesi.
            f_reject: rejected.jsonl dosya nesnesi.

        Returns:
            (başarılı_segment_sayısı, reddedilen_segment_sayısı) tuple'ı.
        """
        try:
            full_audio = (
            ...
            logger.error(f"Dosya yüklenemedi, atlanıyor: {audio_file} - {e}")
            return 0, 0
        ...
        if not segments:
            logger.warning(f"Segment bulunamadı, atlanıyor: {audio_file.name}")
            return 0, 0

        logger.info(f"  -> {len(segments)} segment işlenecek ({audio_file.name})")

        file_success = 0
        file_rejected = 0

        for local_idx, seg in tqdm(
            enumerate(segments),
            total=len(segments),
            desc=f"  {audio_file.name}",
        ):
            start = seg['start']
            end = seg['end']
            ...
            if self.denoiser:
                try:
                    ...
                    logger.error(f"Denoise hatası ({audio_file.name} seg {local_idx}): {e}")

            # Whisper Validation (sadece altyazılı modda)
            if self.validator and expected_text:
                try:
                    is_valid, transcribed, similarity = (
                        self.validator.validate_segment(
                            chunk,
                            expected_text,
                            threshold=self.args.validation_threshold,
                        )
                    )
                    if not is_valid:
                        f_reject.write(json.dumps({
                    ...
                    logger.error(
                        f"Validation hatası ({audio_file.name} seg {local_idx}): {e}"
                    )
            ...
            # Dosya adı: kaynak_dosya + segment numarası
            fname = f"{source_stem}_seg_{local_idx:05d}.wav"
        ...
        logger.info(
            f"  -> {audio_file.name}: "
            f"✓ {file_success} başarılı, ✗ {file_rejected} reddedilen"
        )
        return file_success, file_rejected

    def process(self):
        """Ana işleme metodu. Tek dosya veya klasör girişini destekler.

        Klasör verildiğinde input yapısını output'a yansıtır:
            input/abc/  ->  output/abc/wavs/  + output/abc/metadata.jsonl
            input/xyz/  ->  output/xyz/wavs/  + output/xyz/metadata.jsonl
        Tek dosya verildiğinde ise düz çıktı üretir:
            output/wavs/ + output/metadata.jsonl
        """
        output_root = Path(self.args.out)

        # Ses dosyalarını alt klasörlere göre grupla
        groups = self._collect_audio_groups(self.args.audio)

        if not groups:
            logger.error("İşlenecek ses dosyası bulunamadı!")
            return
        ...
        for group_name, audio_files in sorted(groups.items()):
            # Her grup için ayrı çıktı klasörü oluştur
            if group_name == ".":
                group_output_dir = output_root
            else:
                group_output_dir = output_root / group_name

            clips_dir = group_output_dir / "wavs"
            clips_dir.mkdir(parents=True, exist_ok=True)
            manifest_path = group_output_dir / "metadata.jsonl"
            rejected_path = group_output_dir / "rejected.jsonl"

            logger.info("=" * 60)
            logger.info(
                f"Grup: {group_name} ({len(audio_files)} dosya) "
                f"-> {group_output_dir}"
            )
            ...
            with open(manifest_path, 'w', encoding='utf-8') as f_out, \
                ...
                for audio_file in audio_files:
                    ...
                    logger.info(
                        f"[{processed_file_num}/{total_files}] "
                        f"Ses yükleniyor: {audio_file}"
                    )

                    success, rejected = self._process_single_file(
                        audio_file, clips_dir, f_out, f_reject
                    )
                    total_success += success
                    total_rejected += rejected

        logger.info("=" * 60)
        logger.info(f"TOPLAM: ✓ {total_success} başarılı, ✗ {total_rejected} reddedilen")
        logger.info(f"İşlenen dosya sayısı: {total_files}")
        logger.info(f"İşlenen grup sayısı: {len(groups)}")
        logger.info(f"Çıktı kök klasörü: {output_root}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--audio', required=True,
                        help='Ses/Video dosyası veya WAV dosyaları içeren klasör '
                             '(klasör verilirse alt klasörler dahil tüm .wav dosyaları işlenir)')
    ...
    # Süre Ayarları
    parser.add_argument('--min-sec', type=float, default=1.0, help='Min segment süresi')
    parser.add_argument('--max-sec', type=float, default=15.0, help='Max segment süresi')
