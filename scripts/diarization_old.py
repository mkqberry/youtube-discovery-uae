...
def detect_speakers(
    ...
    Bu fonksiyon hem klasik pyannote Annotation çıktısını
    hem de Transformers diarization (DiarizeOutput / chunks) çıktısını destekler.
    ...
    try:
        ...
        # 1) Klasik pyannote Annotation -> doğrudan Annotation dönerse
        ...
        # 1.b) Yeni pyannote DiarizeOutput -> içinde Annotation taşıyorsa (.annotation)
        ...
        # 1.c) Yeni pyannote speaker_diarization-3.1 -> DiarizeOutput.speaker_diarization (Annotation)
        ...
        # 1.d) Bazı konfigürasyonlarda exclusive_speaker_diarization alanı olabilir
        ...
        # 2) Transformers audio-diarization -> DiarizeOutput / chunks
        elif hasattr(diarization, "chunks"):
            # diarization.chunks tipik olarak:
            # [{ "timestamp": (start, end), "speaker": "SPEAKER_00", ... }, ...]
            for ch in getattr(diarization, "chunks", []):
                ts = ch.get("timestamp") or ch.get("timestamps")
                if not ts or len(ts) != 2:
                    continue
                start, end = ts
                # speaker label bazı modellerde "speaker", bazılarında "label"/"text" olabilir
                speaker = (
                    ch.get("speaker")
                    or ch.get("label")
                    or ch.get("text")
        ...
        # 3) Bazı durumlarda directly list[dict] dönebiliyor
        elif isinstance(diarization, list):
            for ch in diarization:
                ts = ch.get("timestamp") or ch.get("timestamps")
                if not ts or len(ts) != 2:
                    continue
                start, end = ts
            ...
            print(f"  Warning: Unsupported diarization output type: {type(diarization)}")
            return [], 0
...
def process_video_directory(
    ...
    # 1) (legacy) video_dir/metadata.jsonl + video_dir/wavs/*.wav
    # 2) (new)    video_dir/**/*.wav   (no metadata.jsonl required)
    ...
    if has_legacy_layout:
        ...
        if not input_entries:
            print(f"  Warning: No .wav files found under {video_dir.name}, skipping")
            return 0, 0, 0
    ...
    print(f"  Processing {len(input_entries)} audio files...")
    ...
    for entry in input_entries:
        ...
        if num_speakers == 1:
            ...
            # Make sure we have duration (helpful when we didn't start from metadata.jsonl)
            ...
            # Normalize file path to output layout (under output_video_dir/wavs/)
            ...
            for seg_idx, seg in enumerate(grouped_segments):
                ...
                if success:
                    ...
                    new_entry = {
                        ...
                        'text': entry.get('text', ''),  # Keep original text if provided; otherwise empty
        ...
        if total_processed % 10 == 0:
            print(f"    Processed {total_processed}/{len(input_entries)} files...")
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Input directory containing subfolders. Each subfolder can be either:\n"
            "  - legacy layout: metadata.jsonl + wavs/*.wav\n"
            "  - new layout:    *.wav files directly under the folder (any depth)\n"
            "Example: data/input_root"
        )
    )
    parser.add_argument(
        "--output",
