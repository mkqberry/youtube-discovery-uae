...
class Config:
    ...
    INPUT_FILE: Path = Path("./outputs/uae_candidates.jsonl")
    OUTPUT_DIR: Path = Path("arabic_without_subtitle_downloads")
    ...
    MIN_DELAY_BETWEEN_DOWNLOADS: int = 180  # 4 dakika
    MAX_DELAY_BETWEEN_DOWNLOADS: int = 240  # 5 dakika
...
class DownloadManager:
    def __init__(self):
        ...
        # JSONL dosyalarından veri yükle (video_id bazlı)
        self.completed_video_ids = self._load_completed_video_ids()
        self.permanent_fail_video_ids = self._load_permanent_fail_video_ids()
        
        logger.info(f"Daha önce tamamlanan: {len(self.completed_video_ids)} video")
        logger.info(f"Kalıcı hatalı: {len(self.permanent_fail_video_ids)} video")
        
        # Output şablonu
        self.output_tmpl = str(config.OUTPUT_DIR / '%(id)s_%(title).50s' / '%(title)s [%(id)s].%(ext)s')

    def _load_completed_video_ids(self) -> Set[str]:
        """Başarılı indirmeleri yükle (video_id bazlı)"""
        records = jsonl_handler.read_jsonl(config.PROGRESS_FILE)
        video_ids = {r['video_id'] for r in records if r.get('status') == 'success' and r.get('video_id')}
        logger.debug(f"Yüklenen tamamlanmış video ID sayısı: {len(video_ids)}")
        return video_ids

    def _load_permanent_fail_video_ids(self) -> Set[str]:
        """Kalıcı hataları yükle (video_id bazlı)"""
        ...
        logger.debug(f"Yüklenen kalıcı hata video ID sayısı: {len(fails)}")
        return fails

    def _log_success(self, video_id: str, url_data: Dict, file_size: int):
        ...
        record = {
            ...
            'duration_seconds': url_data.get('duration_seconds'),
            'channel_id': url_data.get('channel_id'),
            'decision': url_data.get('decision'),
            'reason_codes': url_data.get('reason_codes'),
    ...
    def _log_fail(self, video_id: str, error: str, error_type: str, url_data: Dict = None):
        """Hata kaydı"""
        url = f"https://www.youtube.com/watch?v={video_id}" if video_id else 'Unknown'
        record = {
            'timestamp': datetime.now().isoformat(),
            'video_id': video_id,
            'error': error,
        ...
        if url_data:
            ...
            record['duration_seconds'] = url_data.get('duration_seconds')
            record['channel_id'] = url_data.get('channel_id')
            record['decision'] = url_data.get('decision')
    ...
    def _get_file_info(self, video_id: str) -> Optional[Tuple[str, int]]:
        ...
        search_pattern = str(config.OUTPUT_DIR / f"{video_id}_*")
    ...
    def download_worker(self, url_data: Dict) -> bool:
        ...
        # Yeni JSONL yapısı: video_id'den URL oluştur
        video_id = url_data.get('video_id', '')
        if not video_id:
            logger.error("video_id bulunamadı, kayıt atlanıyor")
            return False
        ...
        logger.info(f"Süre: {url_data.get('duration_seconds', 'N/A')} saniye")
        logger.info(f"Kanal ID: {url_data.get('channel_id', 'N/A')}")
        logger.info(f"Karar: {url_data.get('decision', 'N/A')}")
        ...
        if file_info is None:
            ...
            self._log_fail(video_id, "Audio dosyası bulunamadı veya geçersiz", "AUDIO_INVALID", url_data)
            stats_tracker.increment('total_processed')
            return False
        
        file_path, file_size = file_info
        logger.info(f"✓ İndirme tamamlandı: {file_path}")
        self._log_success(video_id, url_data, file_size)
        stats_tracker.increment('total_processed')
        ...
        while True:
            ...
            try:
                ...
                if status == "PERMANENT":
                    ...
                    self._log_fail(video_id, msg, "PERMANENT", url_data)
                    stats_tracker.increment('total_processed')
                    return False
                    ...
                    if attempt > config.MAX_RETRIES:
                        ...
                        self._log_fail(video_id, msg, "RETRY", url_data)
                        stats_tracker.increment('total_processed')
                        return False
                ...
                if attempt > config.MAX_RETRIES:
                    self._log_fail(video_id, "Process timeout", "TIMEOUT", url_data)
                    stats_tracker.increment('total_processed')
                    return False
                ...
                self._log_fail(video_id, f"Exception: {str(e)}", "EXCEPTION", url_data)
                stats_tracker.increment('total_processed')
                return False
    ...
    def start(self):
        ...
        if not config.INPUT_FILE.exists():
            ...
            logger.error("Lütfen JSONL dosyası oluşturun")
            logger.error("Format: Her satırda {'video_id': '...', 'title': '...', ...}")
            return

        # JSONL input'u oku
        all_url_data = jsonl_handler.read_jsonl(config.INPUT_FILE)
        logger.info(f"Toplam {len(all_url_data)} video yüklendi")

        # Filtreleme (video_id bazlı)
        queue = []
        skipped = 0
        ...
        for data in all_url_data:
            ...
            if not video_id:
                logger.warning(f"Geçersiz kayıt atlandı (video_id yok): {data.get('title', 'Unknown')}")
                continue
            
            if video_id in self.completed_video_ids:
                logger.debug(f"Atlandı (tamamlanmış): {video_id}")
                skipped += 1
                continue
            
            if video_id in self.permanent_fail_video_ids:
                logger.debug(f"Atlandı (kalıcı hata): {video_id}")
                skipped += 1
                continue
        ...
        logger.info(f"{'='*60}")
        logger.info(f"Toplam Video: {len(all_url_data)}")
        logger.info(f"Daha önce inen: {len(self.completed_video_ids)}")
        logger.info(f"Kalıcı hatalı: {len(self.permanent_fail_video_ids)}")
