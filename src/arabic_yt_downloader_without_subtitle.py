...
class Config:
    ...
    OUTPUT_DIR: Path = Path("data/downloaded/youtube")
    PROGRESS_FILE: Path = Path("logs/arabic_download_without_subtitle_progress.jsonl")
    FAILED_FILE: Path = Path("logs/arabic_download_without_subtitle_failed.jsonl")
    LOG_FILE: Path = Path("logs/arabic_download_without_subtitle_log.txt")
    STATS_FILE: Path = Path("logs/arabic_download_without_subtitle_stats.json")
...
class DownloadManager:
    ...
    def build_command(self, url: str, profile: str) -> List[str]:
        ...
        # Sadece audio indir (en iyi kalite)
        # Format seçimi: bestaudio yoksa alternatif formatlara düşer
        cmd = [
            'yt-dlp',
            '--no-check-certificate',
            # '-f', 'bestaudio[ext=m4a]/bestaudio/best[ext=mp4]/best',  # bestaudio yoksa best formatına düş
            '-f', 'bestaudio',
            '--extract-audio',  # Ses çıkar
            '--audio-format', 'wav',  # MP3 formatına dönüştür (daha uyumlu)
